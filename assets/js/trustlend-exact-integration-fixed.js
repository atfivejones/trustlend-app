...
// Perfect integration for your existing 6-step form
// Maps your exact field IDs to PDF generation system

class TrustLendExactIntegration {
    constructor() {
        // Update this URL when you deploy your API
    ...
    getFullAddress(party) {
        ...
        return `${address}, ${city}, ${state} ${zip}, ${county}`.replace(/,\s*,/g, ',').trim();
    }
            ...
            enhancedSection.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <button onclick="window.trustLendIntegration.generatePDF('contract')" 
                            class="pdf-btn-enhanced bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg flex items-center justify-center gap-2 transition-all">
                    ...
                    <button onclick="window.trustLendIntegration.generatePDF('schedule')" 
                            class="pdf-btn-enhanced bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg flex items-center justify-center gap-2 transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    showStatus(message, isError = false) {
        ...
        if (statusDiv) {
            ...
            statusDiv.className = isError ? 'pdf-status-error' : 'pdf-status-success';
...
// Also create the exact function name your form expects
